...
function Input() {
    ...
    return (
        <>
            {loading ? <BounceLoader className='loader' color="black" size={150} /> :
                <>
                       {/* Stepper */}
                       <ul className="flex items-center justify-between mb-6 relative w-[80%] m-auto mt-5">
  {["Personal Information", "Experience", "Projects"].map((step, idx) => (
    <li
      key={idx}
      className="relative w-1/3 flex flex-col items-center"
    >
      {/* Connector line: Render only for steps after the first */}
      {idx > 0 && (
        <div
          className={`absolute top-1/2 -left-1/2 w-full h-0.5 ${
            currentStep > idx ? "bg-green-600" : "bg-gray-300"
      ...
      >
        {step}
                   ...
                   {/* <div className="input-header ">Enter your details</div> */}
                    <div className="input-main shadow-lg pb-4 rounded-lg">
        ...
        {currentStep === 3 && (
          ...
          <div className="input-head">Personal Projects</div>
                    ...
                    {/* Navigation Buttons */}
<div className="flex justify-between mt-6 w-[60%] m-auto mb-10">
  <button
    type="button"
    onClick={handleBack}
